...
mod extended_tests {
    use crate::{
        ...
        parse_claude_json_line, GitWorktreeInfo
    };
    use crate::mcp_manager::{McpManager, ApprovalRequest, ApprovalResponse};
    ...
    fn test_claude_json_parsing_complex_structures() {
        ...
        let test_cases = vec![
            ...
            (r#"{"type":"assistant","message":{"content":[{"type":"tool_use","name":"write_file","input":{"path":"test.txt","content":"hello"}}]}}"#, None),
            
            // Tool results (should be included)
            (r#"{"type":"assistant","message":{"content":[{"type":"tool_result","content":[{"type":"text","text":"File written successfully"}]}]}}"#, Some("Tool result: File written successfully".to_string())),
            
            // Mixed content (text + tool use)
            (r#"{"type":"assistant","message":{"content":[{"type":"text","text":"I'll write a file"},{"type":"tool_use","name":"write_file","input":{"path":"test.txt"}}]}}"#, Some("I'll write a file".to_string())),
            
            // Tool result with nested structure
            (r#"{"type":"assistant","message":{"content":[{"type":"tool_result","content":{"text":"Nested result"}}]}}"#, Some("Tool result: Nested result".to_string())),
    ...
    fn test_app_state_concurrent_modifications() {
        ...
        for i in 0..50 {
            ...
            let handle = std::thread::spawn(move || {
                ...
                state_clone.worktrees.lock().unwrap().insert(
                    format!("concurrent-{}", i), 
                    worktree
                );
            });
        ...
        for i in 0..50 {
            ...
            let handle = std::thread::spawn(move || {
                ...
                state_clone.processes.lock().unwrap().insert(
                    format!("process-{}", i), 
                    process
                );
            });
    ...
    fn test_state_cleanup_logic() {
        ...
        for i in 0..5 {
            ...
            state.worktrees.lock().unwrap().insert(i.to_string(), worktree);
            state.processes.lock().unwrap().insert(format!("process-{}", i), process);
        }
        ...
        let process_ids_to_remove: Vec<String> = state.processes.lock().unwrap()
            .iter()
        ...
        assert!(!state.worktrees.lock().unwrap().contains_key(target_worktree));
        let remaining_processes = state.processes.lock().unwrap();
        assert!(!remaining_processes.values().any(|p| p.worktree_id == target_worktree));
    }
    ...
    async fn test_mcp_approval_response_handling() {
        ...
        let result = manager.respond_to_approval(approval_id.clone(), allow_response).await;
        assert!(result.is_ok());
    ...
    async fn test_mcp_approval_deny_response() {
        ...
        let result = manager.respond_to_approval(approval_id, deny_response).await;
        assert!(result.is_ok());
    ...
    async fn test_mcp_approval_modified_input() {
        ...
        let result = manager.respond_to_approval(approval_id, modified_response).await;
        assert!(result.is_ok());
    }
    ...
    fn test_timestamp_consistency() {
        ...
        let process_start_time = chrono::DateTime::parse_from_rfc3339(
            process.started_at.as_ref().unwrap()
        ).unwrap();
